...
public class SpawnRunnableAsync extends BukkitRunnable {
    ...
    public void run() {
        ...
        /*Future<ReturnType> futureStatus = scheduler.callSyncMethod(plugin, () -> {
            Player p = Bukkit.getPlayer(playerUUID);
            if (p == null) {
                ConsoleMessage.debug(SpawnRunnableAsync.class, plugin, "Completely stopped runnable because player was null for " + playerUUID);
                return ReturnType.STOP;
            } else if (!worldChecker.isWorldEnabled(p.getWorld())) {
                ConsoleMessage.debug(SpawnRunnableAsync.class, plugin, "world of player " + ConsoleMessage.getPlayerString(p) + " is not enabled");
                return ReturnType.BREAK;
            }
            return ReturnType.PROCEED;
        });

        try {
            ReturnType status = futureStatus.get();
            if (status.equals(ReturnType.PROCEED)) {
                ConsoleMessage.debug(SpawnRunnableAsync.class, plugin, "Proceeding with algorithm...");
                if (spawnAlgorithm.shouldSpawnAsync(playerUUID, plugin)) {
                    int score = scheduler.callSyncMethod(plugin, () -> {
                        Player p = Bukkit.getPlayer(playerUUID);
                        if (p != null) {
                            return obj.getScore(p.getName()).getScore();
                        } else {
                            return -1;
                        }
                    }).get();
                    ...
                    if (score == -1) {
                        ConsoleMessage.debug(this.getClass(), plugin, "Could not proceed with spawning because player is null while determining score. Stopping runnable...");
                        return;
                    }

                    HashMap<Location, Consumer<Phantom>> phantomStatsConsumers = new HashMap<>();
                    for (Location loc : spawnAlgorithm.getSpawnLocationsAsync(playerUUID, plugin)) {
                        phantomStatsConsumers.put(loc, spawnAlgorithm.getPhantomStatsConsumerAsync(score));
                    }

                    scheduler.runTask(plugin, () -> {
                        Player p = Bukkit.getPlayer(playerUUID);
                        if (p == null) {
                            ConsoleMessage.debug(SpawnRunnableAsync.class, plugin, "Could not proceed sync with algorithm because player is null. Stopping runnable...");
                            return;
                        }
                        World world = p.getWorld();
                        HashSet<Phantom> phantomPack = new HashSet<>();

                        for (Location loc : phantomStatsConsumers.keySet()) {
                            ConsoleMessage.debug(this.getClass(), plugin, "Spawning phantom for player " + ConsoleMessage.getPlayerString(p) + "...");
                            Phantom phantom = world.spawn(loc, Phantom.class, phantomStatsConsumers.get(loc));
                            phantom.setTarget(p);
                            phantomPack.add(phantom);
                        }

                        int delay = spawnAlgorithm.getSpawnAttemptDelay();
                        ...
                        SpawnRunnableAsync.startRunnableFromAsync(playerUUID, delay);
                    });
                } else {
                    SpawnRunnableAsync.startRunnableFromAsync(playerUUID);
                }
            } else if (status.equals(ReturnType.BREAK)) {
                ConsoleMessage.debug(SpawnRunnableAsync.class, plugin, "break process because world was not enabled");
                SpawnRunnableAsync.startRunnableFromAsync(playerUUID);
            }
        } catch (InterruptedException | ExecutionException e) {
            ConsoleMessage.debug(this.getClass(), plugin, "Future Get throw error on algorithm check.");
            e.printStackTrace();
        }*/
